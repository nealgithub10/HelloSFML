...
public:
    ...
    //add the command v functionality

    void palette(sf::Event event) {

        if (event.key.system && event.key.code == sf::Keyboard::V) {
             textbox.setString(getText() + sf::Clipboard::getString());
             text.str(getText() +sf::Clipboard::getString());
        }
...
private:
    ...
    void updateBox() {
        // Set the size of the box based on the size of the text
        sf::FloatRect textBounds = textbox.getGlobalBounds();
        outline.setSize(sf::Vector2f(textBounds.width + 6, textBounds.height + 6));
        outline.setOutlineColor(sf::Color::Black);
        outline.setOutlineThickness(2);
        outline.setFillColor(sf::Color::Transparent);
        outline.setPosition(textbox.getPosition() - sf::Vector2f(3, 3));
    }
